...
|---|---|---:|---:|---|---|
| Weak | `google/gemma-3-4b-it` | 4B | $0.04/M input, $0.08/M output | [OpenRouter](https://openrouter.ai/google/gemma-3-4b-it) | Маленькая модель до 6B: дешёвая, лёгкая, но может давать менее полный анализ. |
...
|---|---|---:|---:|---:|---:|---:|---|
| Gemma 3 4B | Weak | 9.12 s | 53 | 513 | 566 | $0.00004316 | Самая дешёвая в этом прогоне, но ответ длиннее и менее сжатый. |
| Llama 3.3 70B Instruct | Medium | 3.38 s | 89 | 233 | 322 | $0.00010477 | Самая быстрая в этом прогоне, ответ компактный и достаточно структурный. |
| Hermes 3 405B Instruct | Strong | 11.23 s | 64 | 281 | 345 | $0.00034500 | Чистый и сжатый ответ, но стоимость выше из-за цены за токен. |

Цифры могут немного меняться от запуска к запуску: OpenRouter маршрутизирует запросы через доступных провайдеров, а модели могут генерировать разную длину ответа. Если провайдер вернул ошибку без `usage`, код явно покажет, что токены не были предоставлены API.
