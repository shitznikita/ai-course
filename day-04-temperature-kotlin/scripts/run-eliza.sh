...
# Day 4 needs a model/endpoint that accepts temperature. Override day-01 defaults if present.
...
export LLM_MODEL="${LLM_MODEL:-deepseek/deepseek-v3.1-terminus}"

cd "$REPO_ROOT"
exec ./gradlew \
    -PelizaTrustStore="$TRUSTSTORE" \
    -PelizaTrustStorePassword="$STORE_PASSWORD" \
    :day-04-temperature-kotlin:run \
    "$@"
