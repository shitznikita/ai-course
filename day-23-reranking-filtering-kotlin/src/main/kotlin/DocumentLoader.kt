...
class DocumentLoader(private val config: AppConfig) {
    ...
    private val allowedNames = setOf(".gitignore")
    ...
    fun load(): LoadedCorpus {
        ...
        val documents = files.mapNotNull { path ->
            ...
            runCatching {
                ...
                if (text.isBlank()) {
                    ...
                    SourceDocument(source, titleFor(path, text), path.extension.lowercase(), text)
                }
            }.getOrElse { error ->
        ...
        return LoadedCorpus(root.toString(), documents, skipped)
    }
    ...
    private fun isAllowed(root: Path, path: Path): Boolean {
        ...
        return path.name in allowedNames || path.extension.lowercase() in allowedExtensions
    }

    private fun extractText(path: Path): String =
        when (path.extension.lowercase()) {
            "pdf" -> Loader.loadPDF(path.toFile()).use { document -> PDFTextStripper().getText(document) }
            else -> path.readText()
        }
