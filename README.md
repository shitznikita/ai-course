...
Day 34 — локальный Kotlin CLI-ассистент, которому задают цель уровня
результата. Он сам обнаруживает и ищет файлы, читает выбранные code/docs,
анализирует их, создаёт или изменяет bounded text files и выводит unified diff.
Embedded Streamable HTTP MCP слушает только loopback и рекламирует ровно пять
tools. Итоговые paths и diff берутся из server-owned session state.

Полная offline-проверка без Ollama:
...
Optional live planner через локальный Ollama:

```bash
ollama serve
ollama pull qwen3:4b
day-34-project-file-assistant-kotlin/scripts/run-file-assistant.sh --args="live-demo"
```

Для произвольного workspace default — preview; реальная запись возможна только
...
- Day 34 не должен отправлять project source в cloud: Ollama и MCP destinations
  обязаны оставаться loopback. Не расширять file tools до shell/delete/rename,
  не ослаблять canonical path/symlink/secret/binary guards и не обходить
