...
class CourseSchedulerAgent(private val config: AppConfig) {
    suspend fun runOnce(label: String = "manual run") {
    ...
    suspend fun runIntervalScheduler() {
        ...
        for (run in 1..config.schedulerRuns) {
            runOnce("interval run $run/${config.schedulerRuns}")
    ...
    suspend fun runDailyScheduler() {
        ...
        while (true) {
            ...
            delay(delayMillis)
            runOnce("daily scheduled run at ${ZonedDateTime.now(config.scheduleZone)}")
        }
    }
