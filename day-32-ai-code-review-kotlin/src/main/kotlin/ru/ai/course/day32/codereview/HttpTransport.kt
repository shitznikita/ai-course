...
data class HttpCall(
    ...
    val maxResponseBytes: Int = 12_000_000,
)
...
class JavaHttpTransport(connectTimeout: Duration = Duration.ofSeconds(15)) : HttpTransport {
    ...
    override fun execute(call: HttpCall): HttpResult {
        ...
        when (call.method) {
            "GET" -> builder.GET()
            "POST" -> builder.POST(HttpRequest.BodyPublishers.ofString(call.body.orEmpty()))
            "PATCH" -> builder.method("PATCH", HttpRequest.BodyPublishers.ofString(call.body.orEmpty()))
            else -> error("Unsupported HTTP method: ${call.method}")
        }
        return try {
