...
same-repository, non-draft pull request
  ...
  -> dedicated self-hosted macOS ARM64 runner with Eliza network access
  -> checkout only immutable base SHA (no credentials)
  -> download Yandex CA bundle and create ignored Java truststore
...
Job требует labels `[self-hosted, macOS, ARM64]`. Runner должен быть выделен
этому репозиторию, работать на доверенной Apple Silicon машине и иметь тот же
сетевой доступ к Eliza, что и локальный live-запуск. Workflow сам запускает
`scripts/setup-yandex-ca.sh`; truststore создаётся в ignored `.certs` внутри
временного checkout и не коммитится. Не используйте этот runner для workflows,
которые checkout-ят или исполняют недоверенный PR head.
...
На машине `macOS ARM64`, где открывается Eliza:

1. Откройте repository `Settings -> Actions -> Runners`.
2. Нажмите `New self-hosted runner`, выберите `macOS` и `ARM64`.
3. Выполните сгенерированные GitHub команды download/configure. Registration
   token одноразовый и ограничен по времени — не сохраняйте и не коммитьте его.
4. Запустите runner командой `./run.sh` и оставьте терминал работающим либо
   установите runner как отдельный macOS service по инструкции GitHub.
5. Убедитесь, что GitHub показывает runner как `Idle` и у него есть labels
   `self-hosted`, `macOS`, `ARM64`.

Runner должен оставаться подключённым к GitHub и к сети/VPN, из которой доступен
`api.eliza.yandex.net`. Если он offline, workflow будет ждать подходящую машину
в очереди. Актуальная инструкция GitHub:
...
repository разрешает такую обработку кода и документации. Self-hosted runner
решает только сетевую доступность Eliza: он не превращает provider в локальную
модель. Если cloud processing запрещён, не добавляйте `LLM_API_KEY` и используйте
отдельный local-provider вариант. MCP здесь не нужен — GitHub REST и trusted base
checkout дают детерминированный контекст с меньшей privileged surface.
