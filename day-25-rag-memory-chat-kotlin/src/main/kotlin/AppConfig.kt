...
data class AppConfig(
    ...
    val chatRecentMessagesInPrompt: Int,
    val chatHistoryMaxMessages: Int,
    ...
    companion object {
        fun load(): AppConfig {
            ...
            return AppConfig(
                ...
                stateDir = Path(env["STATE_DIR"]?.blankToNull() ?: "state").normalize(),
                sessionFile = Path(env["SESSION_FILE"]?.blankToNull() ?: "state/chat-session.json").normalize(),
                reportsDir = Path(env["REPORTS_DIR"]?.blankToNull() ?: "reports").normalize(),
                scenariosFile = Path(env["SCENARIOS_FILE"]?.blankToNull() ?: "eval/scenarios.json").normalize(),
                ...
                chatRecentMessagesInPrompt = env["CHAT_RECENT_MESSAGES_IN_PROMPT"]?.toIntOrNull()?.coerceIn(0, 30) ?: 8,
                chatHistoryMaxMessages = env["CHAT_HISTORY_MAX_MESSAGES"]?.toIntOrNull()?.coerceIn(4, 200) ?: 40,
