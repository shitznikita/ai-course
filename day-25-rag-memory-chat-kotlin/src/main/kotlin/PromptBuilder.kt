class PromptBuilder(private val config: AppConfig) {
    fun groundedMessages(
        ...
        listOf(
            ChatMessage(
                role = "system",
                content = buildString {
                    appendLine("Ты production-like RAG chat assistant по локальной базе знаний проекта ai-course.")
                    appendLine("Отвечай только на основе CONTEXT и QUOTE_CANDIDATES.")
                    appendLine("Учитывай TASK_STATE и RECENT_HISTORY как память диалога, но факты о проекте бери только из CONTEXT.")
            ...
            ChatMessage(
                ...
                content = buildString {
                    ...
                    appendLine(renderTaskState(taskState))
                    appendLine()
                    appendLine("RECENT_HISTORY:")
                    appendLine(renderRecentHistory(recentHistory))
                    appendLine()
    ...
    fun renderGroundedPromptPreview(
        ...
        groundedMessages(question, retrieval, quoteCandidates, taskState, recentHistory).joinToString("\n\n---\n\n") { message ->
            "${message.role.uppercase()}:\n${message.content}"
        }

    private fun renderTaskState(taskState: TaskStateMemory?): String {
        if (taskState == null) return "NO_TASK_STATE"
        return buildString {
            appendLine("goal=${taskState.goal ?: "not set"}")
            appendLine("constraints=${taskState.constraints.joinToString("; ").ifBlank { "none" }}")
            appendLine("terms=${taskState.terms.joinToString(", ").ifBlank { "none" }}")
            appendLine("clarifications=${taskState.clarifications.takeLast(4).joinToString(" | ").ifBlank { "none" }}")
            appendLine("openQuestions=${taskState.openQuestions.takeLast(4).joinToString(" | ").ifBlank { "none" }}")
        }.trim()
    }

    private fun renderRecentHistory(recentHistory: List<ChatTurn>): String =
        recentHistory
            .takeLast(config.chatRecentMessagesInPrompt)
            .joinToString("\n") { turn ->
                "${turn.role}[${turn.turn}]: ${turn.content.shortPreview(260)}"
            }
            .ifBlank { "NO_RECENT_HISTORY" }
