...
OVERRIDE_VARS=(
  ...
  CHAT_RECENT_MESSAGES_IN_PROMPT
...
export SESSION_FILE="${SESSION_FILE:-state/chat-session.json}"
export REPORTS_DIR="${REPORTS_DIR:-reports}"
export SCENARIOS_FILE="${SCENARIOS_FILE:-eval/scenarios.json}"
...
export ANSWER_MIN_RELEVANCE="${ANSWER_MIN_RELEVANCE:-0.55}"
export MIN_QUOTES="${MIN_QUOTES:-1}"
export MAX_QUOTES="${MAX_QUOTES:-4}"
export QUOTE_MAX_CHARS="${QUOTE_MAX_CHARS:-260}"
...
export CHAT_RECENT_MESSAGES_IN_PROMPT="${CHAT_RECENT_MESSAGES_IN_PROMPT:-8}"
export CHAT_HISTORY_MAX_MESSAGES="${CHAT_HISTORY_MAX_MESSAGES:-40}"
...
cd "$ROOT_DIR"
GRADLE_ARGS=("${TRUST_STORE_ARGS[@]}" ":day-25-rag-memory-chat-kotlin:run")
