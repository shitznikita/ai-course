...
```text
фото этикетки -> локальный multi-pass Tesseract -> подтверждённый INCI
текст INCI / точное название из локального каталога
  -> exact ingredient retrieval + allowlist OCR corrections
  -> coverage gate + компактные проверенные facts
  -> qwen3:4b через loopback Ollama /api/chat
  -> strict enum/ID decision + server-side grounding validation
  -> product-level formula signals + claim -> source mapping
  -> серверная сборка отчёта из allowlist-шаблонов и карточек
  -> временный чат с allowlist topic decision
```

Приложение объясняет предполагаемый тип средства, направление распознанной части формулы, возможное место в рутине, полезные ингредиенты и ограничения. Совместимость с кожей больше не получается объединением тегов отдельных ингредиентов: сервис учитывает покрытие состава и прямые совпадения с заполненным профилем аллергий. Он не ставит диагноз, не вычисляет неизвестные концентрации/pH и не объявляет продукт «безопасным» или «токсичным».

Стек: Kotlin/JVM 21, Ktor/CIO, `kotlinx.serialization`, Tesseract OCR и прямой `java.net.http.HttpClient`. Фото распознаётся локальным Tesseract, анализ и чат обращаются только к loopback Ollama. Внешних AI/OCR API и LLM SDK нет.
...
2. **Фото JPEG/PNG** — `/api/ocr` локально возвращает текст Tesseract; пользователь обязан проверить его перед анализом.
...
- `knowledge/ingredient-cards.json`: 63 отдельные карточки распространённых INCI;
- `knowledge/ocr-corrections.json`: восемь точных allowlist-исправлений известных OCR-опечаток без fuzzy guessing;
- `knowledge/sources.json`: 22 первичных/официальных источника EC, EUR-Lex, FDA, SCCS, CIR и точечных clinical studies;
- `catalog/products.json`: четыре вымышленных продукта и демонстрация reformulation;
- `eval/eval-cases.json`: 13 text/name/OCR/unknown/prompt-injection сценариев, включая сырой OCR из проблемного примера и вручную подтверждённую формулу.

Основная регуляторная оговорка: запись ингредиента в CosIng не означает одобрение ингредиента или безопасность готового продукта. Свойства ингредиента нельзя автоматически переносить на готовый продукт без подтверждающих данных. Поэтому CosIng используется для INCI/технических функций, EUR-Lex и SCCS — для точечных регуляторных/safety-ограничений, а clinical study — только с явным указанием конкретной изученной формулы и невозможности переноса эффекта на неизвестную концентрацию.

Большой сторонний «справочник косметики» намеренно не копируется: это смешало бы технические функции, безопасность и efficacy, а условия повторного использования CIR/AAD и коммерческого wINCI не подходят для неограниченного встраивания. Локальная база хранит собственные короткие формулировки и ссылки на первоисточники; UI показывает область применимости рядом с каждой ссылкой.

## Политика качества анализа

- покрытие `< 50%` при составе из трёх и более позиций: LLM не вызывается, chat session не создаётся, применение и совместимость продукта целиком не выводятся;
- покрытие `50–79%`: показываются только осторожные сигналы по распознанной части и явная отметка частичного анализа;
- покрытие `>= 80%`: доступен полный grounded report, но свойства ингредиента всё равно не объявляются доказанным эффектом продукта;
- `presentationRole` и серверная role-policy отделяют benefit от technical/caution: preservatives, fragrance и formulation helpers не заполняют раздел «ключевые ингредиенты»;
- заполненная пользователем аллергия сопоставляется только exact alias/INCI; fuzzy-совпадений нет;
- подтверждённый `BUTYLPHENYL METHYLPROPIONAL` получает отдельную EU-оговорку, но похожий OCR-шум автоматически в Lilial не исправляется;
- каждый отображаемый вывод возвращает собственные `sourceIds`; общий список содержит только реально использованные ссылки;
- лимит карточек относится только к prompt маленькой LLM; детерминированные allergy/regulatory/formula проверки используют все распознанные карточки.
...
- фото передаётся локальному Tesseract через stdin и не создаёт временный файл;
- приложение не отправляет фото, INCI или чат во внешние AI/OCR API;
- список открытых продуктов и их чаты существует только в RAM вкладки; backend-сессии также RAM-only и имеют TTL;
...
VPS остаётся инфраструктурой хостинг-провайдера. Поэтому «приватный» здесь означает закрытый сетевой контур Ollama/Ktor, server-side secrets и отсутствие внешнего AI/OCR, а не абсолютный физический контроль сервера.
...
UI <= 12 products and <= 24 rendered chat messages per product
100 RAM sessions, TTL 30 minutes + periodic RAM cleanup
```

Rate limit и inference queue возвращают `429` и `Retry-After`. Слишком большие или недостаточно распознанные данные отклоняются до Ollama. Все `ingredientId` из решения модели проверяются по текущему evidence pack; citations валидируются на уровне каждого отображаемого claim. Выдуманный ID, чужой источник или лишнее prose-поле превращается в `502`, а пользовательский текст собирается только сервером из allowlist-шаблонов и проверенных данных.
...
Нужны JDK 21, Ollama и Tesseract с языками `eng` и `rus`. Внешние API-ключи не требуются.
...
`fixture-demo` не обращается к модели. `eval-dry-run` проверяет exact retrieval, aliases, актуальную/историческую reformulation, сырой/подтверждённый OCR-состав, неизвестный продукт и prompt injection.
...
Текущий wire contract помечен `apiVersion: "2"`: `highlights`, `skinFit` и `cautions` содержат claim-level `{text, sourceIds}`. Day 30 ещё находится в отдельном pre-release PR, поэтому Ktor и встроенный web UI развёртываются атомарно одной release-директорией; стабильность для сторонних v1-клиентов до merge не обещается. Raw-JSON contract закреплён HTTP-тестом.
...
OCR (ответ дополнительно содержит локальный `provider`, `uncertainFragments` и `notice`):
...
4. Загрузить тестовую этикетку, показать источник OCR и вручную сверить текст, затем запустить локальный анализ.
...
10. Выполнить `ollama ps` и подтвердить, что OCR, рекомендации и чат обрабатываются локально на VPS.
