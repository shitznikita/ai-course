...
```text
фото этикетки -> Eliza Vision (опционально) / multi-pass Tesseract fallback -> подтверждённый INCI
...
Стек: Kotlin/JVM 21, Ktor/CIO, `kotlinx.serialization`, Tesseract OCR и прямой `java.net.http.HttpClient`. Анализ и чат всегда обращаются к локальному Ollama. Опциональный Eliza `gpt-5-mini` используется только как vision/OCR для фотографии и никогда не формирует косметические рекомендации; при его отключении или сетевой ошибке остаётся локальный Tesseract fallback. LLM SDK не используются.
...
2. **Фото JPEG/PNG** — `/api/ocr` возвращает текст и источник распознавания (`eliza_vision` либо локальный fallback); пользователь обязан проверить его перед анализом.
...
Browser -> Caddy HTTPS :443 -> Ktor 127.0.0.1:8787
                              ...
                              -> api.eliza.yandex.net:443 (только фото, опционально)
```
...
- при `ELIZA_VISION_ENABLED=false` фото передаётся Tesseract через stdin и не создаёт временный файл;
- при `ELIZA_VISION_ENABLED=true` фото кодируется в memory-only Base64 и отправляется по HTTPS в Eliza только для транскрипции этикетки; OAuth token остаётся в root-owned server env и не попадает в браузер;
- приложение не сохраняет фото, но для внешнего vision-режима действуют правила обработки данных провайдера; UI явно предупреждает об этом;
...
VPS остаётся инфраструктурой хостинг-провайдера. Поэтому «приватный» здесь означает закрытый сетевой контур Ollama/Ktor, server-side secrets и локальный анализ/чат, а не абсолютный физический контроль. Внешний vision-режим сознательно ослабляет приватность только для загруженной фотографии и может быть полностью выключен.
...
Eliza Vision disabled by default, gpt-5-mini, 90 s timeout
...
Нужны JDK 21, Ollama и Tesseract с языками `eng` и `rus`. Для полностью локального режима больше ничего не требуется. Для опционального Eliza Vision нужен отдельный OAuth token и сеть, из которой доступен `api.eliza.yandex.net`.
...
Eliza Vision по умолчанию выключен. Для осознанного локального запуска можно задать только в ignored `.env`:

```text
ELIZA_VISION_ENABLED=true
ELIZA_VISION_API_KEY=<OAuth token>
ELIZA_VISION_MODEL=gpt-5-mini
```

`ELIZA_VISION_API_URL` жёстко ограничен проверенным HTTPS endpoint, redirects запрещены, ответ принимается только по строгому JSON-контракту. Даже после vision-ответа UI требует ручной проверки INCI.
...
OCR (ответ дополнительно содержит `provider`, `externalProcessing`, `uncertainFragments` и `notice`):
...
Eliza Vision включается только после отдельной сетевой проверки. Endpoint сейчас IPv6-only; если `curl -6` получает timeout, режим надо оставить выключенным — OAuth token не исправляет сетевой ACL:

```bash
curl -6 --connect-timeout 10 \
  https://api.eliza.yandex.net/openrouter/v1/models
```

Даже успешный shell `curl` ещё не разрешает egress приложения: штатный systemd unit намеренно ограничен loopback через `IPAddressDeny=any`. Поэтому на текущем VPS одного изменения env недостаточно и Eliza Vision остаётся выключен. Для будущего внешнего режима нужна отдельная проверенная systemd egress policy под конкретные адреса провайдера. Readiness делает безопасный `GET /models`; если сеть или модель недоступны, фото не отправляется и UI честно показывает локальный fallback.

OAuth token нельзя коммитить, передавать в browser или показывать в видео. Если внешний запрос уже начался, а затем завершился ошибкой, ответ консервативно помечает `externalProcessing=true`: после этого приложение использует Tesseract, но не утверждает, что фото осталось локальным.
...
10. Выполнить `ollama ps` и подтвердить, что рекомендации и чат обрабатывает локальная `qwen3`; отдельно проговорить, включён ли внешний vision только для фото.
