...
private fun serve(config: AppConfig, service: CosmeticsUseCases) {
    ...
    println("PRIVACY: photos, INCI and chat stay on the VPS in RAM; raw Ollama is not exposed.")
...
private fun diagnose(service: CosmeticsUseCases) {
    ...
    println("OCR: ready=${health.ocrReady}, provider=${health.photoOcrProvider}")
...
private fun fixtureDemo(config: AppConfig, knowledge: IngredientKnowledgeBase) {
    ...
    println("Matched fragments: ${pack.matchedFragmentCount}")
    println("Recognized IDs: ${pack.recognized.joinToString { it.card.id }}")
    println("Retrieved source pool (UI shows only claim-cited IDs): ${pack.sources.joinToString { it.id }}")
